...
const updateCategory = async (req, res) => {
  ...
  try {
    const queryText = `
      ...
      RETURNING *
    `;
    const result = await pool.query(queryText, [name, description, id]);
...
// DELETE a category (will set product category_ids to null via database rules)
const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    res.json({
      message:
        "Category deleted successfully. Associated products have been set to Uncategorized.",
    });
  } catch (err) {
    console.error(err);
